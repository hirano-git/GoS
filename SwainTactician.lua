...
SwainMenu.RConfig:Slider("rMana", "Use R if %Mana >", 20, 1, 100, 1)
...
local SwainQ = {delay = 0, range = 700, width = 325, speed = math.huge}
...
if IOW:Mode() == "Combo" then
		...
		if SwainMenu.Combo.W:Value() and IsReady(_W) and ValidTarget(target, 900)then
			...
			if PredictW.hitChance > 0.2 then
				CastSkillShot(_W,PredictW.castPos)
			end
		end
		if SwainMenu.RConfig.R:Value() and IsReady(_R) and ValidTarget(target, 700) and           (GetPercentHP(myHero) <= SwainMenu.RConfig.rHP:Value()) and (GetPercentMP(myHero) >= SwainMenu.RConfig.rMana:Value()) then
			castR(target)
		end
